...
const hasScopeControls = (tab: TabId) => tab === 'overview' || tab === 'global' || tab === 'commercial';
...
export default function AppV8() {
  ...
  const scopeControlsVisible = hasScopeControls(activeTab);
  ...
  return (
    <div className="app-shell app-shell-v8">
      ...
      <main className="workspace workspace-v8">
        ...
        <header className="topbar topbar-v8">
          ...
          <div className="topbar-actions topbar-actions-v8">
          ...
          <>
            ...
            <section className="hero-grid hero-grid-v5">
              <div className="map-panel panel overview-globe-panel">
                <div className="panel-heading"><div><span className="section-kicker">Global opportunity</span><h3>Commercial footprint</h3></div><button className="text-button" onClick={() => changeTab('global')}>Explore rollout →</button></div>
                ...
                <p className="model-note">Drag the globe to rotate it. Tap a modeled country to add it to the active market list. Region colors match the revenue chart.</p>
              </div>
        ...
        {activeTab === 'methodology' && (
          <>
            <section className="panel scenario-file-panel">
              <div>
                <span className="section-kicker">Scenario files</span>
                <h3>Import or export assumptions</h3>
                <p className="model-note">File actions are kept here so they do not occupy the interface on every section.</p>
              </div>
      ...
      {mobileControlsOpen && scopeControlsVisible && (
        <div className="mobile-sheet-backdrop" role="presentation" onClick={() => setMobileControlsOpen(false)}>
          <section className="mobile-controls-sheet" role="dialog" aria-modal="true" aria-label="Markets and indications" onClick={(event) => event.stopPropagation()}>
            <div className="mobile-sheet-header">
              <div><span className="section-kicker">Model scope</span><h3>Markets & indications</h3></div>
              <button className="mobile-sheet-close" onClick={() => setMobileControlsOpen(false)} aria-label="Close model controls">×</button>
            </div>
            <div className="mobile-sheet-scroll">{scopeControls}</div>
          </section>
        </div>
      )}
    </div>
