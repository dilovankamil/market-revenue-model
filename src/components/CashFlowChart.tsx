...
export function CashFlowChart({ data }: CashFlowChartProps) {
  ...
  useEffect(() => {
    ...
    const values = data.flatMap((row) => financingPresent
      ? [row.cumulativeCashFlowUsd, row.cashBalanceUsd]
      : [row.cumulativeCashFlowUsd]);
    ...
    g.append('g')
      ...
      .call((axis) => axis.select('.domain').attr('stroke', '#40505f'))
      .call((axis) => axis.selectAll('text').attr('fill', '#91a1b1'));

    g.append('g')
      .call(d3.axisLeft(y).ticks(5).tickFormat((d) => `$${d3.format('.2s')(Number(d))}`))
    ...
    g.append('line')
      ...
      .attr('stroke', '#718096').attr('stroke-dasharray', '4 4');

    const operatingLine = d3
      .line<YearResult>()
      .x((d) => x(d.year))
      .y((d) => y(d.cumulativeCashFlowUsd))
    ...
    g.append('path')
      ...
      .attr('d', operatingLine);

    if (financingPresent) {
      const balanceLine = d3
        .line<YearResult>()
        .x((d) => x(d.year))
        .y((d) => y(d.cashBalanceUsd))
        .curve(d3.curveMonotoneX);
      g.append('path')
        .datum(data)
        .attr('fill', 'none')
